...
DEFAULT_CONFIG_PATH = "plct-ai-data-unifier-config.yaml"


@cli.command("git-sync")
@click.option("--config", default=DEFAULT_CONFIG_PATH, show_default=True, help="Path to config file (.yaml/.yml/.json)")
...
def git_sync(config, base_dir):
    """Clone or pull repositories listed in a config file.

    The config file should define a top-level "repos" array (YAML or JSON).
    Each item may be either a string (the repo URL) or an object with keys
    `url` and optional `path`.
    """
